...
func GenerateExplanation(query string, modelID string) (*models.ExplanationEntry, error) {
	// Get model from database
	model, err := db.GetLLMModelByID(modelID)
	if err != nil {
	...
	// Get prompt template
	promptTemplate, err := db.GetPromptTemplate()
	if err != nil {
	...
	if promptTemplate == "" {
		...
		if err := db.SavePromptTemplate(promptTemplate); err != nil {
...
// TestPrompt tests a prompt with a model
